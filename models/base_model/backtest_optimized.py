#!/usr/bin/env python3
"""
Backtesting Framework for NY Probability Map - OPTIMIZED VERSION

Tests trading performance with:
- Stop losses at reasonable levels
- Only trading profitable variants
...
# Risk management
STOP_LOSS_POINTS = 40        # Use 40-point stop loss (improved from None)
PROFIT_TARGET_POINTS = None  # None = exit at target level

# Selective variant trading (only trade historically profitable ones)
TRADE_ONLY_PROFITABLE = True
PROFITABLE_VARIANTS = [
...
class BacktestEngine:
    ...
    def should_enter_trade(self, row):
        ...
        # OPTIMIZATION: Only trade profitable variants
        if TRADE_ONLY_PROFITABLE and variant not in PROFITABLE_VARIANTS:
            return None, None
    ...
    def calculate_trade_outcome(self, trade, row):
        """Calculate trade outcome with STOP LOSS logic."""
        # Set target based on direction
        if trade.direction == 'long':
            trade.target_level = row['london_high']
            opposite_level = row['london_low']
            # OPTIMIZATION: Stop loss at fixed distance from entry
            stop_level = trade.entry_price - STOP_LOSS_POINTS if STOP_LOSS_POINTS else opposite_level
        else:  # short
        ...
        if trade.direction == 'long':
            if first_sweep == 'High':
                ...
                # OPTIMIZATION: Exit at stop loss instead of opposite level
    ...
    def run_backtest(self):
        ...
        print(f"  Stop Loss: {STOP_LOSS_POINTS} points" + (" ✓ ENABLED" if STOP_LOSS_POINTS else " (None)"))
        print(f"  Profitable Variants Only: {TRADE_ONLY_PROFITABLE} ({len(PROFITABLE_VARIANTS)} variants)" if TRADE_ONLY_PROFITABLE else "  All Variants")
...
def main():
    ...
    print("\n" + "="*80)
    print("NY PROBABILITY MAP - OPTIMIZED BACKTEST")
    print("="*80)
